...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"GENI" | "USD">("GENI");
  const conversionUnit = editingUnit === "GENI" ? "USD" : "GENI";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "GENI" ? "USD" : "GENI";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "GENI") {
      ...
      setValues({
        GENI: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
