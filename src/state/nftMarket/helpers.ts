...
const fetchCollectionsTotalSupply = async (collections: ApiCollection[], chainId: number): Promise<number[]> => {
  ...
  if (totalSupplyCalls.length > 0) {
    const totalSupplyRaw = await multicallv2(chainId, erc721Abi, totalSupplyCalls, { requireSuccess: false })
 ...
 */
export const getCollections = async (chainId: number): Promise<Record<string, Collection>> => {
  try {
...
export const fetchWalletTokenIdsForCollections = async (
  ...
  const balanceOfCallsResultRaw = await multicallv2(chainId, erc721Abi, balanceOfCalls, { requireSuccess: false })
  ...
  const tokenIdResultRaw = await multicallv2(chainId, erc721Abi, tokenIdCalls, { requireSuccess: false })
...
export const getCompleteAccountNftData = async (
  ...
  const walletNftIdsWithCollectionAddress = await fetchWalletTokenIdsForCollections(account, collections, chainId)
