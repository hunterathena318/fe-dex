...
const masterChefAddress = getMasterChefAddress(ChainId.BSC)
const masterChefContract = getMasterchefContract(ChainId.BSC)
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  const masterChefAggregatedCalls = masterChefCalls
    ...
    .flat()
  const masterChefMultiCallResult = await multicallv2(chainId, masterchefABI, masterChefAggregatedCalls)
