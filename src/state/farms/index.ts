...
export const fetchFarmUserDataAsync = createAsyncThunk<
  ...
  { chainId: number; account: string; pids: number[] },
  {
    state: AppState
  }
>(
  ...
  async ({ chainId, account, pids }) => {
    ...
    const farmsToFetch = farmsConfig.filter((farmConfig) => pids.includes(farmConfig.pid))
    const farmsCanFetch = farmsToFetch.filter((f) => poolLength.gt(f.pid))
    const userFarmAllowances = await fetchFarmUserAllowances(chainId, account, farmsCanFetch)
    const userFarmTokenBalances = await fetchFarmUserTokenBalances(chainId, account, farmsCanFetch)
    const userStakedBalances = await fetchFarmUserStakedBalances(chainId, account, farmsCanFetch)
    const userFarmEarnings = await fetchFarmUserEarnings(chainId, account, farmsCanFetch)
