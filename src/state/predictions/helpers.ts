...
export const getLedgerData = async (account: string, epochs: number[], chainId: number) => {
  ...
  const response = await multicallv2(chainId, predictionsAbi, ledgerCalls)
  return response
}
...
export const getClaimStatuses = async (
  ...
  const claimableResponses = await multicallv2(chainId, predictionsAbi, claimableCalls)
...
export const getRoundsData = async (epochs: number[], chainId: number): Promise<PredictionsRoundsResponse[]> => {
  const address = getPredictionsAddress()
  ...
  const response = await multicallv2(chainId, predictionsAbi, calls)
  return response
}
...
export const fetchUsersRoundsLength = async (account: string, chainId: number) => {
  try {
