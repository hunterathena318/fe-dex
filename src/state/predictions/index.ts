...
>
export const initializePredictions = createAsyncThunk<PredictionInitialization, { account: string; chainId: number }>(
  'predictions/initialize',
  async ({ account = null, chainId }) => {
    // Static values
    ...
    // Round data
    const roundsResponse = await getRoundsData(epochs, chainId)
    ...
    // Bet data
    const ledgerResponses = await getLedgerData(account, epochs, chainId)

    // Claim statuses
    const claimableStatuses = await getClaimStatuses(account, epochs, chainId)
...
export const fetchRound = createAsyncThunk<ReduxNodeRound, { epoch: number; chainId: number }>(
  'predictions/fetchRound',
  async ({ epoch, chainId }) => {
...
export const fetchRounds = createAsyncThunk<{ [key: string]: ReduxNodeRound }, { epochs: number[]; chainId: number }>(
  'predictions/fetchRounds',
  async ({ epochs, chainId }) => {
    const rounds = await getRoundsData(epochs, chainId)
...
export const fetchMarketData = createAsyncThunk<MarketData, { chainId: number }>(
  'predictions/fetchMarketData',
  async ({ chainId }) => {
    const marketData = await getPredictionData(chainId)
    return marketData
  },
...
export const fetchLedgerData = createAsyncThunk<LedgerData, { account: string; epochs: number[]; chainId: number }>(
  'predictions/fetchLedgerData',
  async ({ account, epochs, chainId }) => {
    const ledgers = await getLedgerData(account, epochs, chainId)
...
export const fetchClaimableStatuses = createAsyncThunk<
  ...
  { account: string; epochs: number[]; chainId: number }
>('predictions/fetchClaimableStatuses', async ({ account, epochs, chainId }) => {
  const ledgers = await getClaimStatuses(account, epochs, chainId)
  return ledgers
})
...
export const fetchNodeHistory = createAsyncThunk<
  ...
  { account: string; page?: number; chainId: number }
>('predictions/fetchNodeHistory', async ({ account, page = 1, chainId }) => {
  const userRoundsLength = await fetchUsersRoundsLength(account, chainId)
  ...
  const size =
    maxPages === page
      ? userRoundsLength
          ...
          .toNumber()
      : ROUNDS_PER_PAGE
  const userRounds = await fetchUserRounds(account, cursor.lt(0) ? 0 : cursor.toNumber(), size, chainId)
  ...
  const roundData = await getRoundsData(epochs, chainId)
  const claimableStatuses = await getClaimStatuses(account, epochs, chainId)
