...
export const fetchPublicIfoPoolData = async (chainId: number) => {
  try {
    const calls = [
    ...
    const [[sharePrice], [shares], [estimatedCakeBountyReward], [totalPendingCakeHarvest], [startBlock], [endBlock]] =
      await multicallv2(chainId, ifoPoolAbi, calls)
...
export const fetchIfoPoolFeesData = async (chainId: number) => {
  try {
