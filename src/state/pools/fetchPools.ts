...
export const fetchPoolsBlockLimits = async (chainId) => {
  const startEndBlockRaw = await multicall(chainId, sousChefABI, startEndBlockCalls)
...
export const fetchPoolsTotalStaking = async (chainId: number) => {
  const poolsTotalStaked = await multicall(chainId, erc20ABI, poolsBalanceOf)
...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingResultRaw = await multicallv2(chainId, sousChefV2, poolStakingCalls, { requireSuccess: false })
...
export const fetchPoolsProfileRequirement = async (
  ...
  const poolProfileRequireResultRaw = await multicallv2(chainId, sousChefV3, poolProfileRequireCalls, {
