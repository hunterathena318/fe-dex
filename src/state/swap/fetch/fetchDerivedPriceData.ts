import { getInfoClientUrl, getTimestampInfo } from 'config/constants/endpoints'
...
const fetchDerivedPriceData = async (
  ...
  const endTimestamp = getUnixTime(new Date(getTimestampInfo(chainId)))
  ...
  try {
    ...
    const token0DerivedBnb = await getTokenDerivedBnbPrices(token0Address, blocks, chainId)
    const token1DerivedBnb = await getTokenDerivedBnbPrices(token1Address, blocks, chainId)
