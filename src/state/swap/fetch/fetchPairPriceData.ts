...
const fetchPairPriceData = async ({ pairId, timeWindow, chainId }: fetchPairDataParams) => {
  ...
  console.log({ pairId, timeWindow, chainId })
  try {
    switch (timeWindow) {
      case PairDataTimeWindowEnum.DAY: {
        const data = await requestWithTimeout<PairHoursDatasResponse>(client(chainId), pairHourDatas, {
          pairId,
      ...
      case PairDataTimeWindowEnum.WEEK: {
        const lastPairHourIdData = await requestWithTimeout<LastPairHourIdResponse>(client(chainId), lastPairHourId, {
          pairId,
        })
        ...
        const pairHoursData = await requestWithTimeout<PairHoursDatasResponse>(client(chainId), pairHourDatasByIds, {
          pairIds: pairHourIds,
        })
        return { data: pairHoursData, error: false }
      }
      case PairDataTimeWindowEnum.MONTH: {
        const data = await requestWithTimeout<PairHoursDatasResponse>(client(chainId), pairDayDatas, {
          pairId,
          first: timeWindowIdsCountMapping[timeWindow],
        })
        return { data, error: false }
      ...
      case PairDataTimeWindowEnum.YEAR: {
        const lastPairDayIdData = await requestWithTimeout<LastPairDayIdResponse>(client(chainId), lastPairDayId, {
          pairId,
        })
        ...
        const pairDayData = await requestWithTimeout<PairDayDatasResponse>(client(chainId), pairDayDatasByIdsQuery, {
          pairIds: pairDayIds,
        })
