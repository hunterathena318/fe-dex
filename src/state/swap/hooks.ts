...
import { DEFAULT_INPUT_CURRENCY_MAP, DEFAULT_OUTPUT_CURRENCY_MAP } from './constants'
...
export function useDerivedSwapInfo(
  ...
  if (!parsedAmount) {
    inputError = inputError ?? t('Confirm Order')
  }
  ...
  if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
    inputError = t('Insufficient %symbol% balance', { symbol: amountIn.currency.getSymbol(chainId) })
  }
...
export function queryParametersToSwapState(parsedQs: ParsedUrlQuery, chainId: number): SwapState {
  let inputCurrency = parseCurrencyFromURLParameter(parsedQs.inputCurrency) || DEFAULT_INPUT_CURRENCY_MAP[chainId]
  let outputCurrency = parseCurrencyFromURLParameter(parsedQs.outputCurrency) || DEFAULT_OUTPUT_CURRENCY_MAP[chainId]
    ...
    const fetchDerivedData = async () => {
      ...
      try {
        ...
        if (derivedData) {
          ...
          console.log({ derivedData, normalizedDerivedData })
  ...
  useEffect(() => {
    const updatePairId = () => {
      try {
        ...
        let myToken1 = token1
        if (token0 === ETHER) {
          myToken0 = WETH[chainId]
        }

        if (token1 === ETHER) {
          myToken1 = WETH[chainId]
        }
        const pairAddress = getLpAddress(myToken0, myToken1)?.toLowerCase()
    ...
    updatePairId()
  }, [token0Address, token1Address, pairId, token0, token1, chainId])
