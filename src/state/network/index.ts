...
const chainIdSafeCheck = (): { chainId: number; chainIdFromUrl: boolean } => {
  ...
  if (chainStr) {
    ...
    if (chainStr.toLowerCase() === NETWORK_LABEL[CHAIN_ID.MAINNET].toLowerCase()) {
      return { chainId: CHAIN_ID.MAINNET, chainIdFromUrl: true }
    }
    if (chainStr.toLowerCase() === NETWORK_LABEL[CHAIN_ID.AVALANCHE].toLowerCase()) {
      return { chainId: CHAIN_ID.AVALANCHE, chainIdFromUrl: true }
    }
