...
import { getGraphAPILottery } from 'config/constants/endpoints'
...
const getLotteriesData = async (currentLotteryId: string, chainId: number): Promise<LotteryRoundGraphEntity[]> => {
  const idsForNodesCall = getRoundIdsArray(currentLotteryId)
  const nodeData = await fetchMultipleLotteries(idsForNodesCall, chainId)
  const graphResponse = await getGraphLotteries(undefined, undefined, undefined, chainId)
  const mergedData = applyNodeDataToLotteriesGraphResponse(nodeData, graphResponse)
  return mergedData
}

export const getPriceGemuniBusd = async () => {
  // new BigNumber(0)
  // return useSelector(priceCakeFromPidSelector)
  const response = await axios.get(`https://api.coingecko.com/api/v3/simple/price/?ids=gemuni&vs_currencies=usd`)
  const price = response.data.gemuni.usd
  return price
}
