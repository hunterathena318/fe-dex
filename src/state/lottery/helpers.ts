...
export const fetchMultipleLotteries = async (lotteryIds: string[], chainId: number): Promise<LotteryResponse[]> => {
  ...
  try {
    const multicallRes = await multicallv2(chainId, lotteryV2Abi, calls, { requireSuccess: false })
...
export const fetchCurrentLotteryIdAndMaxBuy = async (chainId: number) => {
  try {
