...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@techchainswapfinance/sdk'
...
const getErrorMessage = (
  ...
  if (currencyBalances.input && currencyBalances.input.lessThan(parsedAmounts.input)) {
    return `Insufficient ${currencyBalances.input.currency.getSymbol(trade.route.chainId)} balance`
  }
