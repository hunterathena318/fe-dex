...
`
export const BallNumber = styled.img`
  width: 100px;
  height: 100px;
  margin-right: 15px;
`
...
export const BallWithNumber: React.FC<BallWithNumberProps> = ({ color, number, size, fontSize, rotationTransform }) => {
  ...
  const showNumberBall = (num: string) => {
    return <BallNumber src={`/images/lottery/icons/ball_${num}.svg`} />
  }
  return <div style={{ marginRight: '10px' }}>{showNumberBall(number)}</div>
}
