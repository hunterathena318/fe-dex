...
const StyledCardBody = styled(CardBody)`
  ...
  min-height: 240px;
`
const BuyTicketsButtonGradient = styled(BuyTicketsButton)`
  background: linear-gradient(90deg, #06d3e6 -0.15%, #ffdb0d 96.99%) !important;
  font-size: 16px;
`
...
const YourHistoryCard: React.FC<YourHistoryCardProps> = ({ handleShowMoreClick, numUserRoundsRequested }) => {
  ...
  const getHeader = () => {
    ...
    return (
      <Heading scale="md" fontSize="18px">
        {t('Rounds')}
      </Heading>
    )
  }
      ...
      return (
        <StyledCardBody>
          <Box maxWidth="280px">
            ...
            <Text textAlign="center" color="#BBB5CE" mb="16px">
              {t('Buy tickets for the next round!')}
            </Text>
            <BuyTicketsButtonGradient disabled={ticketBuyIsDisabled} width="100%" />
          </Box>
        </StyledCardBody>
      )
    }
  ...
  const getFooter = () => {
    ...
    return (
      <CardFooter style={{ borderColor: 'rgba(129, 75, 246, 0.4)' }}>
  ...
  return (
    <StyledCard>
      <CardHeader variant="lottery">{getHeader()}</CardHeader>
      {getBody()}
      {getFooter()}
    </StyledCard>
  )
