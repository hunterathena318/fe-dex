...
const PreviousRoundTicketsInner: React.FC<{ roundId: string }> = ({ roundId }) => {
  ...
  const TooltipComponent = () => (
    <>
      <Text mb="16px" color='#000000'>
        {t('Tickets must match the winning number in the exact same order, starting from the first digit.')}
      </Text>
      <Text mb="16px"color='#000000'>{t('If the winning number is “123456”:')}</Text>
      <Text mb="4px" color='#000000'>{t('“120000” matches the first 2 digits.')}</Text>
      <Text color='#000000'>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const userTickets = await fetchUserTicketsForOneRound(account, roundId, chainId)
    ...
    fetchData()
  }, [roundId, account, totalTickets, chainId])

  const getFooter = () => {
    if (userWinningTickets?.ticketsWithUnclaimedRewards?.length > 0) {
      return (
        <Button onClick={onPresentClaimModal} variant="linear" mt="24px" width="100%">
          {t('Collect Prizes')}
        </Button>
      )
    }
