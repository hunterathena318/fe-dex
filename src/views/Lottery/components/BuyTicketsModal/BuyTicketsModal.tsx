...
import { fetchCurrentLotteryIdAndMaxBuy } from 'state/lottery/helpers'
import useTheme from 'hooks/useTheme'
import useTokenBalance from 'hooks/useTokenBalance'
import { FetchStatus } from 'config/constants/types'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { useGemuni, useLotteryV2Contract } from 'hooks/useContract'
import { useCallWithGasPrice } from 'hooks/useCallWithGasPrice'
import useToast from 'hooks/useToast'
import ConnectWalletButton from 'components/ConnectWalletButton'
import { ToastDescriptionWithTx } from 'components/Toast'
import ApproveConfirmButtons, { ButtonArrangement } from 'components/ApproveConfirmButtons'
import { getGemuniTokenAddress } from 'utils/addressHelpers'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const { reader: cakeContractReader, signer: cakeContractApprover } = useGemuni()
  const { toastSuccess } = useToast()
  const { balance: userCake, fetchStatus } = useTokenBalance(getGemuniTokenAddress(chainId))
  ...
  const TooltipComponent = () => (
    <>
      ...
      <Text color="#110733">{t('2 tickets: 0.05%')}</Text>
      <Text color="#110733">{t('50 tickets: 2.45%')}</Text>
      <Text color="#110733">{t('100 tickets: 4.95%')}</Text>
    </>
  )
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      ...
      // If the users' max GENI balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  const getNumTicketsByPercentage = (percentage: number): number => {
    const percentageOfMaxTickets = new BigNumber(percentage)
    //  maxPossibleTicketPurchase.gt(0)
  ...
  const MAX_TICKETS_CAN_BUY = 100

  const handleInputChange = (input: string) => {
    // Force input to integer
    const inputAsInt = parseInt(input, 10)

    if (inputAsInt > oneHundredPercentOfBalance) {
      setUserNotEnoughCake(true)
    }

    if (inputAsInt >= MAX_TICKETS_CAN_BUY) {
      setUserNotEnoughCake(false)
      setMaxTicketPurchaseExceeded(true)
      setTicketsToBuy(MAX_TICKETS_CAN_BUY.toString())
  ...
  const { isApproving, isApproved, isCheckApproving, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      ...
      onConfirm: async () => {
        ...
        const currentLottery = await fetchCurrentLotteryIdAndMaxBuy(chainId)
        const tx = await callWithGasPrice(lotteryContract, 'buyTickets', [
          currentLottery.currentLotteryId,
          ticketsForPurchase,
        ])
        addTransaction(tx, {
          summary: `Buy ${ticketsToBuy} Tickets From Lottery`,
        })
        return tx
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient GENI balance')
    return t('The maximum number of tickets you can buy in one transaction is %maxTickets%', {
      maxTickets: MAX_TICKETS_CAN_BUY,
    })
  }
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInCake.times(new BigNumber(ticketsToBuy))) : '0.00'} GENI`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCake && getFullDisplayBalance(priceTicketInCake.times(ticketsToBuy || 0))} GENI
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
        ...
        <Flex borderTop={`1px solid ${theme.colors.cardBorder}`} pt="8px" mb="24px" justifyContent="space-between">
          <Text color="textSubtle">{t('You pay')}</Text>
        ...
        {account ? (
          <>
            <ApproveConfirmButtons
              ...
              isConfirming={isConfirming}
              isCheckApproving={isCheckApproving}
