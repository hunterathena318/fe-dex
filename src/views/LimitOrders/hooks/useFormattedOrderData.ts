...
import { Currency, CurrencyAmount, Price, Token, TokenAmount } from '@techchainswapfinance/sdk'
...
const useFormattedOrderData = (order: Order): FormattedOrderData => {
  ...
  return {
    ...
    bscScanUrls: {
      created: order.createdTxHash ? getEtherscanLink(order.createdTxHash, 'transaction', chainId) : null,
      executed: order.executedTxHash ? getEtherscanLink(order.executedTxHash, 'transaction', chainId) : null,
      cancelled: order.cancelledTxHash ? getEtherscanLink(order.cancelledTxHash, 'transaction', chainId) : null,
    },
  }
}
