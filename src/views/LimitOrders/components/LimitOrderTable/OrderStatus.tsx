...
const OpenTextColor = styled.div`
  & div {
...
`
const CancelTextColor = styled.div`
  & div {
...
const OrderStatus: React.FC<{
  ...
  if (isOpen && !isSubmissionPending && showOpenTag) {
    return (
      <OpenTextColor>
        <StatusElement element={element} text={t('Open')} color="success" />
      </OpenTextColor>
    )
  }
  ...
  if (isCancelled && !isCancellationPending) {
    return (
      <CancelTextColor>
        <StatusElement element={element} text={t('Cancelled')} color="failure" />
      </CancelTextColor>
    )
  }
