...
export const DetailLimitOrderModal: React.FC<DetailLimitOrderModalProps> = ({ onDismiss, order, formattedOrder }) => {
  ...
  const onCancelOrder = useCallback(() => {
    ...
    const orderDetails =
      formattedOrder.inputToken?.getSymbol(chainId) &&
      formattedOrder.outputToken?.getSymbol(chainId) &&
      formattedOrder.inputAmount &&
      formattedOrder.outputAmount
        ? {
            inputTokenSymbol: formattedOrder.inputToken?.getSymbol(chainId),
            outputTokenSymbol: formattedOrder.outputToken?.getSymbol(chainId),
    ...
    formattedOrder.inputToken,
    formattedOrder.outputToken,
    formattedOrder.inputAmount,
    formattedOrder.outputAmount,
    chainId,
    order,
  ...
  const limitPriceExchangeRateText = `1 ${formattedOrder.inputToken?.getSymbol(chainId)} = ${
    formattedOrder.executionPrice
  } ${formattedOrder.outputToken?.getSymbol(chainId)}`
  const limitPriceExchangeRateTextReversed = `1 ${formattedOrder.outputToken?.getSymbol(chainId)} = ${
    formattedOrder.invertedExecutionPrice
  } ${formattedOrder.inputToken?.getSymbol(chainId)}`
    ...
    <>
      <Flex width="100%" justifyContent="space-between">
        <CellFormat
          ...
          secondRow={<CurrencyFormat currency={formattedOrder.inputToken} chainId={chainId} />}
        />
        <ChevronRightIcon />
        <CellFormat
          firstRow={<Text>{formattedOrder.outputAmount}</Text>}
          secondRow={<CurrencyFormat currency={formattedOrder.outputToken} chainId={chainId} />}
        />
      </Flex>
      <LimitTradeInfoCard
      ...
      <Flex flexDirection="column">
        {isOpen ? (
          <>
            <Button variant="linear" mt="16px" as="a" external href={formattedOrder.bscScanUrls.created}>
              {t(`View on  Explorer`)}
            </Button>
            {!isSubmissionPending && (
              <Button
          ...
          <Button variant="linear" mt="16px" as="a" external href={formattedOrder.bscScanUrls.created}>
            {t(`View order creation on Explorer`)}
          </Button>
        )}
        {isCancelled && bscScanUrls.cancelled && (
          <Button variant="linear" mt="16px" as="a" external href={bscScanUrls.cancelled}>
            {t(`View order cancellation on Explorer`)}
          </Button>
        )}
        {isExecuted && bscScanUrls.executed && (
          <Button variant="linear" mt="16px" as="a" external href={bscScanUrls.executed}>
            {t(`View order execution on Explorer`)}
          </Button>
        )}
      </Flex>
    </>
  ...
  return (
    <Modal
      ...
      style={{ width: '436px', padding: '0 10px' }}
      onDismiss={onDismiss}
    >
      {attemptingTxn ? (
        <LoadingContent />
      ...
      <InfoCardWrapper>
        <Box mb="4px">
          ...
          {isCancelled && !isCancellationPending && (
            <CancelTag outline scale="sm" p="8px" mb="16px" variant="failure">
              {t('Cancelled')}
            </CancelTag>
          )}
