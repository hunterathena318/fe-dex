    ...
    >
      ...
      {!txHash && !swapErrorMessage && (
        <Flex justifyContent="center" mt="24px">
          <Button scale="md" variant="linear" onClick={onConfirm} width="100%" disabled={attemptingTxn}>
...
const OrderContent: React.FC<OrderContentProps> = ({
  ...
  const { t } = useTranslation()
  const { chainId } = useActiveWeb3React()
  const currentPriceExchangeRateText = `1 ${currencies.input?.getSymbol(
    chainId,
  )} = ${currentMarketRate} ${currencies.output?.getSymbol(chainId)}`
  const currentPriceExchangeRateTextReversed = `1 ${currencies.output?.getSymbol(
    chainId,
  )} = ${currentMarketRateInverted} ${currencies.input?.getSymbol(chainId)}`
  const limitPriceExchangeRateText = `1 ${currencies.input?.getSymbol(
    chainId,
  )} = ${limitPrice} ${currencies.output?.getSymbol(chainId)}`
  const limitPriceExchangeRateTextReversed = `1 ${currencies.output?.getSymbol(
    chainId,
  )} = ${limitPriceInverted} ${currencies.input?.getSymbol(chainId)}`
  return (
    <>
      <Flex flexDirection="column" width="100%">
        <Flex alignItems="center" justifyContent="space-between">
          ...
          <Flex alignItems="center">
            <Text color="text" mr="8px">
              {currencies.input?.getSymbol(chainId)?.toUpperCase()}
            </Text>
        ...
        <Flex alignItems="center" justifyContent="space-between">
          ...
          <Flex alignItems="center">
            <Text color="text" mr="8px">
              {currencies.output?.getSymbol(chainId)?.toUpperCase()}
            </Text>
  ...
  return (
    <Flex style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <Flex flexDirection="column" flexGrow="wrap" flexBasis="267px">
        <TextGradient fontSize="20px" small color="secondary">
          {t('Confirm')}
        </TextGradient>
