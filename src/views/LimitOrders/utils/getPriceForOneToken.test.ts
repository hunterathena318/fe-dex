...
describe('limitOrders/utils/getPriceForOneToken', () => {
  describe.each([
    [new TokenAmount(GENI, ONE), new TokenAmount(BUSD, ONE), '1'],
    [new TokenAmount(GENI, FIVE), new TokenAmount(BUSD, FIVE), '1'],
    [new TokenAmount(GENI, ONE), new TokenAmount(BUSD, FIVE), '5'],
    [new TokenAmount(GENI, FIVE), new TokenAmount(BUSD, ONE), '0.2'],
    ...
    [new TokenAmount(GENI, ZERO), new TokenAmount(BUSD, ONE), undefined],
    [new TokenAmount(GENI, ONE), new TokenAmount(BUSD, ZERO), undefined],
  ])(`returns correct price`, (input, output, expected) => {
    it(`for ${input.toSignificant(6)} ${input.currency.getSymbol(chainId)} -> ${output.toSignificant(
      6,
    )} ${output.currency.getSymbol(chainId)}`, () => {
