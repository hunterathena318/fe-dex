...
export default function RemoveLiquidity() {
  ...
  async function onAttemptToApprove() {
    ...
    console.log({ chainId, name: DOMAIN_LP_MAP[chainId], DOMAIN_LP_MAP })
    const domain = {
      name: DOMAIN_LP_MAP[chainId],
      version: '1',
      chainId,
  ...
  async function onRemove() {
      ...
      if (oneCurrencyIsETH) {
        methodNames =
          chainId === ChainId.AVALANCHE
            ? ['removeLiquidityAVAX', 'removeLiquidityAVAXSupportingFeeOnTransferTokens']
    ...
    // we have a signature, use permit versions of remove liquidity
      ...
      if (oneCurrencyIsETH) {
        methodNames =
          chainId === ChainId.AVALANCHE
            ? ['removeLiquidityAVAXWithPermit', 'removeLiquidityAVAXWithPermitSupportingFeeOnTransferTokens']
  ...
  const pendingText = t('Removing %amountA% %symbolA% and %amountB% %symbolB%', {
    ...
    symbolA: currencyA?.getSymbol(chainId) ?? '',
    amountB: parsedAmounts[Field.CURRENCY_B]?.toSignificant(6) ?? '',
    symbolB: currencyB?.getSymbol(chainId) ?? '',
  })
      ...
      <AppBody>
        ...
        <CardBody>
          <AutoColumn gap="20px">
            <RowBetween>
              <Text>{t('Amount')}</Text>
              <GradientTextButton variant="text" scale="sm" onClick={() => setShowDetailed(!showDetailed)}>
          ...
          <Box position="relative" mt="16px">
            {!account ? (
              ...
              <RowBetween>
                <Button
                  variant={approval === ApprovalState.APPROVED || signatureData !== null ? 'success' : 'linear'}
