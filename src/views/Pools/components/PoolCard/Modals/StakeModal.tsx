        ...
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('Your %symbol% earnings have also been harvested to your wallet!', {
              symbol: earningToken?.getSymbol(chainId),
            })}
        ...
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('Your %symbol% funds have been staked in the pool!', {
              symbol: stakingToken?.getSymbol(chainId),
            })}
          </ToastDescriptionWithTx>,
        )
      }
  ...
  if (showRoiCalculator) {
    return (
      <RoiCalculatorModal
        ...
        apr={apr}
        linkLabel={t('Get %symbol%', { symbol: stakingToken?.getSymbol(chainId) })}
        linkHref={getTokenLink}
        stakingTokenBalance={userData.stakedBalance.plus(stakingTokenBalance)}
        stakingTokenSymbol={stakingToken?.getSymbol(chainId)}
        earningTokenSymbol={earningToken?.getSymbol(chainId)}
      ...
      {stakingLimit.gt(0) && !isRemovingStake && (
        <Text color="secondary" bold mb="24px" style={{ textAlign: 'center' }} fontSize="16px">
          {t('Max stake for this pool: %amount% %token%', {
            ...
            token: stakingToken?.getSymbol(chainId),
          })}
        </Text>
      )}
      <Flex alignItems="center" justifyContent="space-between" mb="8px">
        ...
        <Flex alignItems="center" minWidth="70px">
          <Image
            ...
            width={24}
            height={24}
            alt={stakingToken?.getSymbol(chainId)}
          />
          <Text ml="4px" bold>
            {stakingToken?.getSymbol(chainId)}
          </Text>
        </Flex>
      </Flex>
      <BalanceInput
      ...
      {!isRemovingStake && (
        <StyledLink external href={getTokenLink}>
          <Button width="100%" mt="8px" variant="secondary">
            {t('Get %symbol%', { symbol: stakingToken?.getSymbol(chainId) })}
          </Button>
        </StyledLink>
      )}
    </Modal>
