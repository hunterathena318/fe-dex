...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  } = useTooltip(t('Total amount of %symbol% staked in this pool', { symbol: stakingToken.getSymbol(chainId) }), {
    placement: 'bottom',
  })
        ...
        {totalStaked && totalStaked.gte(0) ? (
          <>
            <Balance
              ...
              decimals={0}
              unit={` ${stakingToken.getSymbol(chainId)}`}
            />
        ...
        {account && isMetaMaskInScope && tokenAddress && (
          <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
            <Button
              ...
              p="0"
              height="auto"
              onClick={() => registerToken(tokenAddress, earningToken.getSymbol(chainId), earningToken.decimals)}
            >
        ...
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {vaultKey
              ...
              : `${t('Earn')} GENI ${t('Stake').toLocaleLowerCase()} GENI`}
          </Text>
        )}
        {pool.vaultKey ? (
