...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of GENI up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max GENI entry is capped by minimum between your average GENI balance in the IFO GENI pool, or the pool’s hard cap. To increase the max entry, Stake more GENI into the IFO GENI pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max GENI entry is capped by your average GENI balance in the IFO GENI pool. To increase the max entry, Stake more GENI into the IFO GENI pool',
        )
      : t(
          'For the unlimited sale, Max GENI entry is capped by your average GENI balance in the IFO GENI pool. To increase the max entry, Stake more GENI into the IFO GENI pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. GENI entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('GENI to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
