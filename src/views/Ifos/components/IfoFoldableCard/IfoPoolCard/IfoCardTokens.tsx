...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough GENI to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your GENI.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate Gemuni Exchange Profile to take part in next IFO‘s!')
                : t('You need an active Gemuni Exchange Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (
      ...
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average GENI balance available to commit in the IFO GENI pool.')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, stake some GENI in the IFO GENI pool!')}
              </Text>
