  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Stake GENI in IFO pool')}
      </Heading>
      <Box>
        <Text color="textSubtle" small>
          {t(
            'The maximum amount of GENI user can commit to the Public Sale, is equal to the average GENI balance in the IFO GENI pool prior to the IFO. Stake more GENI to increase the maximum GENI you can commit to the sale. Missed this IFO? You can keep staking in the IFO GENI Pool to join the next IFO sale.',
          )}
        </Text>
      ...
      {hasProfile && (
        <SmallStakePoolCard borderRadius="default" p="16px">
          <FlexGap justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="16px">
            ...
            {isVaultApproved ? (
              <Button onClick={onPresentStake}>{t('Stake')} GENI</Button>
            ) : (
...
const Step2 = ({ hasProfile, isLive, isCommitted }: { hasProfile: boolean; isLive: boolean; isCommitted: boolean }) => {
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can “commit” your GENI to buy the tokens being sold.')} <br />
      </Text>
        ...
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small mb="16px">
              {t('You’ll need an active Gemuni Exchange Profile to take part in an IFO!')}
            </Text>
            {renderAccountStatus()}
          </CardBody>
        )
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent GENI tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
