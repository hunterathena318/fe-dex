...
const ResponsiveGrid = styled.div`
  ...
  grid-gap: 1em;
  align-items: center;
  grid-template-columns: 20px 2fr repeat(2, 1.5fr) 2fr 1.5fr 1fr;
    ...
    <LinkWrapper to={`/info/pool/${poolData.address}`}>
      <ResponsiveGrid>
        ...
        <Flex ml="15px">
          ...
          <AlignText ml="8px" fontWeight="600" style={{ display: 'flex' }}>
            {poolData.token0.symbol}/{' '}
            <AlignText fontWeight="600" style={{ color: '#807e9f' }}>
              {poolData.token1.symbol}
            </AlignText>
          </AlignText>
        </Flex>
        <AlignText>{`$ ${formatAmount(poolData.volumeUSD)}`}</AlignText>
        <AlignText>{`$ ${formatAmount(poolData.volumeUSDWeek)}`}</AlignText>
        <AlignText>{`$ ${formatAmount(poolData.lpFees24h)}`}</AlignText>
        <AlignText>{formatAmount(poolData.lpApr7d)}%</AlignText>
        <AlignText>{`$ ${formatAmount(poolData.liquidityUSD)}`}</AlignText>
      </ResponsiveGrid>
    </LinkWrapper>
  )
}
...
const PoolTable: React.FC<PoolTableProps> = ({ poolDatas, loading }) => {
  ...
  return (
    <TableWrapper>
      <ResponsiveGrid>
        <AlignText color="#777E90" fontSize="16px" fontFamily="Inter">
          #
        </AlignText>
        <AlignText color="#777E90" fontSize="16px" fontFamily="Inter" textTransform="uppercase">
          {t('Pool')}
        </AlignText>
