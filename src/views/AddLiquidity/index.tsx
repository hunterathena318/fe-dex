...
export default function AddLiquidity() {
    ...
    if (currencyA === ETHER || currencyB === ETHER) {
      ...
      estimate =
        chainId === ChainId.AVALANCHE
          ? routerContract.estimateGas.addLiquidityAVAX
          : routerContract.estimateGas.addLiquidityETH
      method = chainId === ChainId.AVALANCHE ? routerContract.addLiquidityAVAX : routerContract.addLiquidityETH
      args = [
  ...
  const pendingText = t('Supplying %amountA% %symbolA% and %amountB% %symbolB%', {
    ...
    symbolA: currencies[Field.CURRENCY_A]?.getSymbol(chainId) ?? '',
    amountB: parsedAmounts[Field.CURRENCY_B]?.toSignificant(6) ?? '',
    symbolB: currencies[Field.CURRENCY_B]?.getSymbol(chainId) ?? '',
  })
        ...
        <CardBody>
          <AutoColumn gap="20px">
            ...
            {currencies[Field.CURRENCY_A] && currencies[Field.CURRENCY_B] && pairState !== PairState.INVALID && (
              <>
                <LightCard padding="0px" background="#532b95" borderRadius="20px">
                    ...
                    <RowBetween>
                      {approvalA !== ApprovalState.APPROVED && (
                        ...
                        >
                          {approvalA === ApprovalState.PENDING ? (
                            <Dots>
                              {t('Enabling %asset%', { asset: currencies[Field.CURRENCY_A]?.getSymbol(chainId) })}
                            </Dots>
                          ) : (
                            t('Enable %asset%', { asset: currencies[Field.CURRENCY_A]?.getSymbol(chainId) })
                          )}
                        </Button>
                      )}
                      {approvalB !== ApprovalState.APPROVED && (
                        ...
                        >
                          {approvalB === ApprovalState.PENDING ? (
                            <Dots>
                              {t('Enabling %asset%', { asset: currencies[Field.CURRENCY_B]?.getSymbol(chainId) })}
                            </Dots>
                          ) : (
                            t('Enable %asset%', { asset: currencies[Field.CURRENCY_B]?.getSymbol(chainId) })
                          )}
                        </Button>
                      )}
                    </RowBetween>
