...
const ProfilePicture: React.FC = () => {
  ...
  const { nfts, isLoading: isUserNftLoading } = useNftsForAddress(account, profile, isProfileLoading, chainId)
    ...
    const fetchUserPancakeCollectibles = async () => {
      try {
        ...
        if (nftsByCollection.length > 0) {
          ...
          const collectionRolesRaw = await multicall(chainId, profileABI, collectionsNftRoleCalls)
