...
import { Currency, currencyEquals, ETHER, Token } from '@techchainswapfinance/sdk'
...
//           (tokenMap_: { [x: string]: any }, token: { address: string | number }) => {
...
export function useAllTokens(): { [address: string]: Token } {
  const tokenMap = useSelector(combinedTokenMapFromActiveUrlsSelector)
...
export function useOfficialsAndUserAddedTokens(): { [address: string]: Token } {
  const tokenMap = useSelector(combinedTokenMapFromOfficialsUrlsSelector)
  const userAddedTokens = useSelector(userAddedTokenSelector)
  const { chainId } = useActiveWeb3React()
  return useMemo(
    () =>
      userAddedTokens
