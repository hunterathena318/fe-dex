...
import { GELATO_HANDLER, getGelatoHandler } from 'config/constants'
...
const useGelatoLimitOrdersLib = (): GelatoLimitOrders | undefined => {
  ...
  return useMemo(() => {
    ...
    try {
      return new GelatoLimitOrders(chainId as ChainId, library?.getSigner(), getGelatoHandler(chainId), false)
    } catch (error: any) {
