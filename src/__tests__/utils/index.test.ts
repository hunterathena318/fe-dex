...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@techchainswapfinance/sdk'
import { getEtherscanLink, calculateSlippageAmount, isAddress, calculateGasMargin, basisPointsToPercent } from 'utils'

describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getEtherscanLink('abc', 'transaction', ChainId.BSC)).toEqual('https://bscscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getEtherscanLink('abc', 'token', ChainId.BSC)).toEqual('https://bscscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getEtherscanLink('abc', 'address', ChainId.BSC)).toEqual('https://bscscan.com/address/abc')
    })
    it('enum', () => {
      expect(getEtherscanLink('abc', 'address', ChainId.BSC_TESTNET)).toEqual('https://testnet.bscscan.com/address/abc')
    })
  })
