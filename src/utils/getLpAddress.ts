import { Token, Pair, ChainId } from '@techchainswapfinance/sdk'
...
const getLpAddress = (token1: string | Token, token2: string | Token) => {
  ...
  console.log({ token1AsTokenInstance, token2AsTokenInstance })
  if (!token1 || !token2) {
    return null
  }
  if (typeof token1 === 'string' || token1 instanceof String) {
    ...
    token1AsTokenInstance = new Token(ChainId.BSC, checksummedToken1Address, 18)
  }
  if (typeof token2 === 'string' || token2 instanceof String) {
    ...
    token2AsTokenInstance = new Token(ChainId.BSC, checksummedToken2Address, 18)
  }
