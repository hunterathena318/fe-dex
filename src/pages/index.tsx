...
export { default } from 'views/Swap'

const IndexPage = () => {
  const { chainId } = useActiveWeb3React()
  const [data, setData] = useState({
    totalTx30Days: 0,
    addressCount30Days: 0,
    tvl: 0,
  })
    ...
    const fetchData = async () => {
      ...
      if (process.env.SF_HEADER) {
        try {
          ...
          const totalTx = await infoServerClient(chainId).request(totalTxQuery, {
            id: FACTORY_ADDRESS_MAP[chainId],
          })
          const totalTx30DaysAgo = await infoServerClient(chainId).request(totalTxQuery, {
            block: {
  ...
  return (
    <SWRConfig
      value={{
        fallback: {
          totalTx30Days: data.addressCount30Days,
          addressCount30Days: data.addressCount30Days,
          tvl: data.tvl,
        },
      }}
    >
...
//       const totalTx = await infoServerClient(ChainId.BSC).request(totalTxQuery, {
//         id: FACTORY_ADDRESS,
//       })
//       const totalTx30DaysAgo = await infoServerClient(ChainId.BSC).request(totalTxQuery, {
//         block: {
//           number: days30AgoBlock.number,
//         },
//         id: FACTORY_ADDRESS,
