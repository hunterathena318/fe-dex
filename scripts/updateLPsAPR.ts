...
import { BLOCKS_CLIENT, getBlockClient } from '../src/config/constants/endpoints'
...
const getBlockAtTimestamp = async (timestamp: number, chainId: number) => {
  try {
...
const fetchAndUpdateLPsAPR = async (chainId: number) => {
  ...
  const lowerCaseAddresses = farmsConfig
    ...
    .map((farm) => farm.lpAddresses[ChainId.BSC].toLowerCase())
